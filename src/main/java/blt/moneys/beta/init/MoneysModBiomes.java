...
public class MoneysModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, MoneysMod.MODID);
	public static final RegistryObject<Biome> MONEY_BIOME = REGISTRY.register("money_biome", () -> MoneyBiomeBiome.createBiome());
			...
			for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
				...
				if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.OVERWORLD_LOCATION)) {
					...
					if (chunkGenerator.getBiomeSource()instanceof MultiNoiseBiomeSource noiseSource) {
						...
						parameters.add(new Pair<>(MoneyBiomeBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, MONEY_BIOME.getId()))));
					...
					if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
						...
						if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
							...
							surfaceRules.add(1, overworldRule(ResourceKey.create(Registry.BIOME_REGISTRY, MONEY_BIOME.getId()),
									Blocks.GRASS_BLOCK.defaultBlockState(), Blocks.STONE.defaultBlockState()));
