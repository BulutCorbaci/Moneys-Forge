...
import blt.moneys.beta.network.ReburnerBetaGUISlotMessage;
...
public class ReburnerBetaGUIMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	public ReburnerBetaGUIMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		super(MoneysModMenus.REBURNER_BETA_GUI, id);
		...
		this.customSlots.put(1, this.addSlot(new SlotItemHandler(internal, 1, 16, 53) {
			@Override
			public boolean mayPlace(ItemStack stack) {
				return (Items.BLACK_DYE == stack.getItem());
			}
		}));
	...
	private void slotChanged(int slotid, int ctype, int meta) {
		if (this.world != null && this.world.isClientSide()) {
			MoneysMod.PACKET_HANDLER.sendToServer(new ReburnerBetaGUISlotMessage(slotid, x, y, z, ctype, meta));
			ReburnerBetaGUISlotMessage.handleSlotAction(entity, slotid, ctype, meta, x, y, z);
		}
	}
